...
AAmethystWeapon::AAmethystWeapon(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
void AAmethystWeapon::OnEquip(const AAmethystWeapon* LastWeapon)
{
	AttachMeshToPawn();
...
void AAmethystWeapon::UseAmmo()
{
	...
	AAmethystAIController* BotAI = MyPawn ? Cast<AAmethystAIController>(MyPawn->GetController()) : NULL;
	AAmethystPlayerController* PlayerController = MyPawn ? Cast<AAmethystPlayerController>(MyPawn->GetController()) : NULL;
	if (BotAI)
	{
		BotAI->CheckAmmo(this);
	}
...
void AAmethystWeapon::HandleFiring()
{
	...
	else if (MyPawn && MyPawn->IsLocallyControlled())
	{
		if (GetCurrentAmmo() == 0 && !bRefiring)
		{
			...
			AAmethystPlayerController* MyPC = Cast<AAmethystPlayerController>(MyPawn->Controller);
...
{
	AAmethystPlayerController* const PlayerController = Instigator ? Cast<AAmethystPlayerController>(Instigator->Controller) : NULL;
...
{
	AAmethystPlayerController* PC = MyPawn ? Cast<AAmethystPlayerController>(MyPawn->Controller) : NULL;
...
void AAmethystWeapon::SimulateWeaponFire()
{
	...
	AAmethystPlayerController* PC = (MyPawn != NULL) ? Cast<AAmethystPlayerController>(MyPawn->Controller) : NULL;
...
{
	const AAmethystPlayerController* MyPC = (MyPawn != NULL) ? Cast<const AAmethystPlayerController>(MyPawn->Controller) : NULL;
