...
#include "AmethystForestSession.h"
...
AAmethystForestSession::AAmethystForestSession(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &AAmethystForestSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &AAmethystForestSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &AAmethystForestSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &AAmethystForestSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &AAmethystForestSession::OnStartOnlineGameComplete);
	}
}
 ...
 */
void AAmethystForestSession::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful)
{
...
void AAmethystForestSession::HandleMatchHasStarted()
{
 ...
 *
 */
void AAmethystForestSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
bool AAmethystForestSession::IsBusy() const
{ 
...
EOnlineAsyncTaskState::Type AAmethystForestSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
 ...
 */
const TArray<FOnlineSessionSearchResult> & AAmethystForestSession::GetSearchResults() const
{ 
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AAmethystForestSession::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AAmethystForestSession::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
...
bool AAmethystForestSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
	...
	if (OnlineSub)
	{
		...
		{
			HostSettings = MakeShareable(new FAmethystOnlineSessionSettings(bIsLAN, bIsPresence, MaxPlayers));
...
void AAmethystForestSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void AAmethystForestSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void AAmethystForestSession::ChooseBestSession()
{
...
void AAmethystForestSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void AAmethystForestSession::ContinueMatchmaking()
{	
	ChooseBestSession();
...
void AAmethystForestSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void AAmethystForestSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid() && CurrentSessionParams.UserId.IsValid())
		{
			SearchSettings = MakeShareable(new FAmethystOnlineSearchSettings(bIsLAN, bIsPresence));
...
bool AAmethystForestSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool AAmethystForestSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
 ...
 * @param bWasSuccessful true if the async action completed without error, false if there was an error
 */
void AAmethystForestSession::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool AAmethystForestSession::TravelToSession(int32 ControllerId, FName SessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
