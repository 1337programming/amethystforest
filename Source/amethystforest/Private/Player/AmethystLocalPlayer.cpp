...
UAmethystLocalPlayer::UAmethystLocalPlayer(const FObjectInitializer& ObjectInitializer)
...
void UAmethystLocalPlayer::LoadPersistentUser()
{
	...
	if (PersistentUser == NULL)
	{
		...
		PersistentUser = UAmethystPersistentUser::LoadPersistentUser( GetNickname(), PlatformId );
	}
}
...
FString UAmethystLocalPlayer::GetNickname() const
{
	...
	{
		bReentry = true;
		UAmethystForestInstance* GameInstance = GetWorld() != NULL ? Cast<UAmethystForestInstance>(GetWorld()->GetGameInstance()) : NULL;
		if(GameInstance)
		{
