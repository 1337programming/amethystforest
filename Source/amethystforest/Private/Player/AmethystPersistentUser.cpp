...
UAmethystPersistentUser::UAmethystPersistentUser(const FObjectInitializer& ObjectInitializer)
...
bool UAmethystPersistentUser::IsAimSensitivityDirty() const
{
	...
	if (GEngine)
	{
		...
		for (auto It = PlayerList.CreateIterator(); It; ++It)
		{
			...
			UAmethystLocalPlayer* LocalPlayer = Cast<UAmethystLocalPlayer>(PC->Player);
...
void UAmethystPersistentUser::SetIsRecordingDemos(const bool InbIsRecordingDemos)
{
