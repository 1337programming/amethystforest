...
AAmethystHUD::AAmethystHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	OnPlayerTalkingStateChangedDelegate = FOnPlayerTalkingStateChangedDelegate::CreateUObject(this, &AAmethystHUD::OnPlayerTalkingStateChanged);
...
void AAmethystHUD::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	ConditionalCloseScoreboard(true);

	AAmethystPlayerController* AmethystPC = Cast<AAmethystPlayerController>(PlayerOwner);
	if (AmethystPC != NULL )
	{
		// Reset the ignore input flags, so we can control the camera during warmup
		AmethystPC->SetCinematicMode(false,false,false,true,true);
	}

	Super::EndPlay(EndPlayReason);
}

void AAmethystHUD::SetMatchState(EAmethystMatchState::Type NewState)
{
	MatchState = NewState;
}

EAmethystMatchState::Type AAmethystHUD::GetMatchState() const
{
	return MatchState;
}
...
void AAmethystHUD::DrawMatchTimerAndPosition()
{
	...
	if (MyGameState && MatchState == EAmethystMatchState::Playing)
	{
	...
	if (MyGameState && MyGameState->RemainingTime > 0)
	{
		...
		Text = FString();
		AAmethystPlayerController* MyPC = Cast<AAmethystPlayerController>(PlayerOwner);
		if (MyPC && MyGameState && MatchState == EAmethystMatchState::Playing)
		{
...
{
	AAmethystPlayerController* PCOwner = Cast<AAmethystPlayerController>(PlayerOwner);
	if (PCOwner)
	{
			...
			&& (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
		{
			...
			AAmethystWeapon_Instant* InstantWeapon = Cast<AAmethystWeapon_Instant>(Pawn->GetWeapon());
...
void AAmethystHUD::ShowDeathMessage(class AAmethystPlayerState* KillerPlayerState, class AAmethystPlayerState* VictimPlayerState, const UDamageType* KillerDamageType)
{
	...
	if (GetWorld()->GameState && GetWorld()->GameState->GameModeClass)
	{
		...
		AAmethystPlayerState* MyPlayerState = PlayerOwner ? Cast<AAmethystPlayerState>(PlayerOwner->PlayerState) : NULL;
		...
		if (DefGame && KillerPlayerState && VictimPlayerState && MyPlayerState)
		{
			...
			NewMessage.DamageType = Cast<const UAmethystDamageType>(KillerDamageType);
...
void AAmethystHUD::OnPlayerTalkingStateChanged(TSharedRef<const FUniqueNetId> TalkingPlayerId, bool bIsTalking)
{
...
void AAmethystHUD::PostInitializeComponents()
{
...
void AAmethystHUD::ConditionalCloseScoreboard(bool bFocus)
{
	if (bIsScoreBoardVisible)
	{
...
void AAmethystHUD::ToggleScoreboard()
{
	ShowScoreboard(!bIsScoreBoardVisible);
}

bool AAmethystHUD::ShowScoreboard(bool bEnable, bool bFocus)
{
	...
	{
		AAmethystPlayerController* AmethystPC = Cast<AAmethystPlayerController>(PlayerOwner);
		if (AmethystPC == NULL || AmethystPC->IsGameMenuVisible() )
		{
			return false;
		}
	}
	...
	if (bIsScoreBoardVisible)
	{
		...
		.Padding(FMargin(50))
		[
			SAssignNew(ScoreboardWidget, SAmethystScoreboardWidget)
...
void AAmethystHUD::ToggleChat()
{
	AAmethystPlayerController* AmethystPC = Cast<AAmethystPlayerController>(PlayerOwner);
	// If the game menu is visible dont show the chat menu
	if( (AmethystPC == NULL ) || AmethystPC->IsGameMenuVisible() || GetMatchState() == EAmethystMatchState::Warmup )
	{
		return;
	}
...
void AAmethystHUD::SetChatVisibilty( const EVisibility RequiredVisibility )
{
	TryCreateChatWidget();
...
void AAmethystHUD::AddChatLine(const FText& ChatString, bool bWantFocus)
{
	TryCreateChatWidget();
...
bool AAmethystHUD::TryCreateChatWidget()
{
	bool bCreated = false;
	AAmethystPlayerController* AmethystPC = Cast<AAmethystPlayerController>(PlayerOwner);
	if(AmethystPC == NULL )
	{
		UE_LOG(LogAmethyst, Warning, TEXT("Unable to create chat widget - Invalid player controller") );
	}
	else
	{
		...
		{
			bCreated = true;
			FLocalPlayerContext WorldContext(AmethystPC);
			GEngine->GameViewport->AddViewportWidgetContent(
...
bool AAmethystHUD::IsMatchOver() const
{
	return GetMatchState() == EAmethystMatchState::Lost || GetMatchState() == EAmethystMatchState::Won;
}
...
float AAmethystHUD::DrawRecentlyKilledPlayer()
{		
