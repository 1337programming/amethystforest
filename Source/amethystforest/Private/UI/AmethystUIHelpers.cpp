...
FText AmethystUIHelpers::GetProfileOpenText() const
{
...
bool AmethystUIHelpers::ProfileOpenedUI(const FUniqueNetId& Requestor, const FUniqueNetId& Requestee, const IOnlineExternalUI::FOnProfileUIClosedDelegate* Delegate) const
{
...
FText AmethystUIHelpers::GetProfileSwapText() const
{
	// @todo: replace button with icon
//#if PLATFORM_XBOXONE
...
bool AmethystUIHelpers::ProfileSwapUI(const int ControllerIndex, bool bShowOnlineOnly, const IOnlineExternalUI::FOnLoginUIClosedDelegate* Delegate) const
{
	const auto OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
