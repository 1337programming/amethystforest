...
DECLARE_DELEGATE_RetVal_OneParam(int32, FOnGetPlayerStateAttribute, AAmethystPlayerState*);
...
//class declare
class SAmethystScoreboardWidget : public SBorder
{

	SLATE_BEGIN_ARGS(SAmethystScoreboardWidget)
	{}

	SLATE_ARGUMENT(TWeakObjectPtr<APlayerController>, PCOwner)

	SLATE_ATTRIBUTE(EAmethystMatchState::Type, MatchState)
...
protected:
	...
	AAmethystPlayerState* GetSortedPlayerState(const FTeamPlayer& TeamPlayer) const;
	...
	int32 GetAttributeValue_Kills(class AAmethystPlayerState* PlayerState) const;

	/** get attribute value for deaths */
	int32 GetAttributeValue_Deaths(class AAmethystPlayerState* PlayerState) const;

	/** get attribute value for score */
	int32 GetAttributeValue_Score(class AAmethystPlayerState* PlayerState) const;
	...
	const struct FAmethystScoreboardStyle *ScoreboardStyle;
};
