...
void SAmethystMenuWidget::Construct(const FArguments& InArgs)
{
	...
	const FText ProfileSwap = AmethystUIHelpers::Get().GetProfileSwapText();
	const TSharedRef< FSlateFontMeasure > FontMeasure = FSlateApplication::Get().GetRenderer()->GetFontMeasureService();
	const FSlateFontInfo PlayerNameFontInfo = FAmethystStyle::Get().GetWidgetStyle<FTextBlockStyle>("AmethystForest.MenuProfileNameStyle").Font;
	const FSlateFontInfo ProfileSwapFontInfo = FAmethystStyle::Get().GetWidgetStyle<FTextBlockStyle>("AmethystForest.MenuServerListTextStyle").Font;
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(SImage)
					.Visibility(this, &SAmethystMenuWidget::GetProfileSwapVisibility)
			...
			[
				...
				[
					SNew(STextBlock)
					.TextStyle(FAmethystStyle::Get(), "AmethystForest.MenuProfileNameStyle")
...
FText SAmethystMenuWidget::GetMenuTitle() const
{
...
bool SAmethystMenuWidget::ProfileUISwap(const int ControllerIndex) const
{
	if(IsProfileSwapActive())
	{
		...
		{
			UAmethystForestInstance* GameInstance = PlayerOwner.IsValid() ? Cast< UAmethystForestInstance >( PlayerOwner->GetGameInstance() ) : nullptr;
...
void SAmethystMenuWidget::HandleProfileUISwapClosed(TSharedPtr<const FUniqueNetId> UniqueId, const int ControllerIndex)
{
	UAmethystForestInstance * GameInstance = PlayerOwner.IsValid() ? Cast< UAmethystForestInstance >( PlayerOwner->GetGameInstance() ) : nullptr;
...
int32 SAmethystMenuWidget::GetMenuLevel()
{
...
FText SAmethystMenuWidget::GetOptionText(TSharedPtr<FAmethystMenuItem> MenuItem) const
{
