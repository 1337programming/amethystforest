...
void SAmethystServerList::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			...
			[
				...
				.OnGenerateRow(this, &SAmethystServerList::MakeListViewWidget)
				.OnSelectionChanged(this, &SAmethystServerList::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&SAmethystServerList::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &SAmethystServerList::GetBottomText)
 ...
 */
AAmethystForestSession* SAmethystServerList::GetGameSession() const
{
...
void SAmethystServerList::UpdateSearchStatus()
{
	check(bSearchingForServers); // should not be called otherwise

	bool bFinishSearch = true;
	AAmethystForestSession* AmethystSession = GetGameSession();
	if (AmethystSession)
	{
		int32 CurrentSearchIdx, NumSearchResults;
		EOnlineAsyncTaskState::Type SearchState = AmethystSession->GetSearchResultStatus(CurrentSearchIdx, NumSearchResults);

		UE_LOG(LogOnlineGame, Log, TEXT("AmethystSession->GetSearchResultStatus: %s"), EOnlineAsyncTaskState::ToString(SearchState) );
		...
		switch(SearchState)
		{
			...
			case EOnlineAsyncTaskState::Done:
				...
				{
					ServerList.Empty();
					const TArray<FOnlineSessionSearchResult> & SearchResults = AmethystSession->GetSearchResults();
...
FText SAmethystServerList::GetBottomText() const
{
	 return StatusText;
}
 ...
 */
void SAmethystServerList::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
void SAmethystServerList::BeginServerSearch(bool bLANMatch, const FString& InMapFilterName)
{
...
void SAmethystServerList::OnServerSearchFinished()
{
	bSearchingForServers = false;
...
void SAmethystServerList::UpdateServerList()
{
...
void SAmethystServerList::ConnectToServer()
{
	if (bSearchingForServers)
	{
		// unsafe
...
void SAmethystServerList::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply SAmethystServerList::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void SAmethystServerList::EntrySelectionChanged(TSharedPtr<FServerEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void SAmethystServerList::OnListItemDoubleClicked(TSharedPtr<FServerEntry> InItem)
{
	SelectedItem = InItem;
	ConnectToServer();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void SAmethystServerList::MoveSelection(int32 MoveBy)
{
	int32 SelectedItemIndex = ServerList.IndexOfByKey(SelectedItem);
...
FReply SAmethystServerList::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
TSharedRef<ITableRow> SAmethystServerList::MakeListViewWidget(TSharedPtr<FServerEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
