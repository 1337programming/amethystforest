...
//class declare
class SAmethystMenuWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SAmethystMenuWidget)
	: _PlayerOwner()
	, _IsGameMenu(false)
	{}
	...
	void UpdateArrows(TSharedPtr<FAmethystMenuItem> MenuItem);
	...
	TArray<FAmethystMenuInfo> MenuHistory;
...
private:
	...
	FText GetOptionText(TSharedPtr<FAmethystMenuItem> MenuItem) const;
	...
	const struct FAmethystMenuStyle *MenuStyle;
};

namespace MenuHelper
{
	FORCEINLINE void EnsureValid(TSharedPtr<FAmethystMenuItem>& MenuItem)
	{
		if (!MenuItem.IsValid())
		{
			MenuItem = FAmethystMenuItem::CreateRoot();
		}
	}

	//Helper functions for creating menu items
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddMenuItem(TSharedPtr<FAmethystMenuItem>& MenuItem, const FText& Text)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FAmethystMenuItem> Item = MakeShareable(new FAmethystMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddMenuItem(TSharedPtr<FAmethystMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FAmethystMenuItem::FOnConfirmMenuItem::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FAmethystMenuItem> Item = MakeShareable(new FAmethystMenuItem(Text));
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddMenuItemSP(TSharedPtr<FAmethystMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FAmethystMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FAmethystMenuItem> Item = MakeShareable(new FAmethystMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddMenuOption(TSharedPtr<FAmethystMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FAmethystMenuItem> Item = MakeShareable(new FAmethystMenuItem(Text, OptionsList));
		MenuItem->SubMenu.Add(Item);
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddMenuOption(TSharedPtr<FAmethystMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FAmethystMenuItem::FOnOptionChanged::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FAmethystMenuItem> Item = MakeShareable(new FAmethystMenuItem(Text, OptionsList));
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddMenuOptionSP(TSharedPtr<FAmethystMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FAmethystMenuItem::FOnOptionChanged::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FAmethystMenuItem> Item = MakeShareable(new FAmethystMenuItem(Text, OptionsList));
	...
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddExistingMenuItem(TSharedPtr<FAmethystMenuItem>& MenuItem, TSharedRef<FAmethystMenuItem> SubMenuItem)
	{
		EnsureValid(MenuItem);
	...
	FORCEINLINE TSharedRef<FAmethystMenuItem> AddCustomMenuItem(TSharedPtr<FAmethystMenuItem>& MenuItem, TSharedPtr<SWidget> CustomWidget)
	{
		EnsureValid(MenuItem);
		MenuItem->SubMenu.Add(MakeShareable(new FAmethystMenuItem(CustomWidget)));
		return MenuItem->SubMenu.Last().ToSharedRef();
	}

	FORCEINLINE void ClearSubMenu(TSharedPtr<FAmethystMenuItem>& MenuItem)
	{
		EnsureValid(MenuItem);
	...
	template< class UserClass >	
	FORCEINLINE void PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, int32 UserIndex, UserClass* inObj, typename FAmethystMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
