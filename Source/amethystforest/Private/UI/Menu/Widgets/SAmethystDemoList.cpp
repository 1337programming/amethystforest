...
void SAmethystDemoList::Construct(const FArguments& InArgs)
{
	...
	[
		...
		[
			SNew(SCheckBox)
			.IsChecked(this, &SAmethystDemoList::IsShowAllReplaysChecked)
			.OnCheckStateChanged(this, &SAmethystDemoList::OnShowAllReplaysChecked)
			.Style(FCoreStyle::Get(), "Checkbox")
			[
				SNew(STextBlock)
				.Text(LOCTEXT("ShowAllDemos", "Show demos from all versions"))
				.TextStyle(FAmethystStyle::Get(), "AmethystForest.DemoListCheckboxTextStyle")
			]
		]
		+SVerticalBox::Slot()
		.AutoHeight()
		[
			...
			[
				...
				.OnGenerateRow(this, &SAmethystDemoList::MakeListViewWidget)
				.OnSelectionChanged(this, &SAmethystDemoList::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&SAmethystDemoList::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(STextBlock)
				.Text(this, &SAmethystDemoList::GetBottomText)
				.TextStyle(FAmethystStyle::Get(), "AmethystForest.MenuServerListTextStyle")
			]
		]
	];
...
void SAmethystDemoList::OnEnumerateStreamsComplete(const TArray<FNetworkReplayStreamInfo>& Streams)
{
...
FText SAmethystDemoList::GetBottomText() const
{
	 return StatusText;
}
 ...
 */
void SAmethystDemoList::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	SCompoundWidget::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
}

ECheckBoxState SAmethystDemoList::IsShowAllReplaysChecked() const
{
...
void SAmethystDemoList::OnShowAllReplaysChecked(ECheckBoxState NewCheckedState)
{
	EnumerateStreamsVersion = FNetworkVersion::GetReplayVersion();
...
void SAmethystDemoList::BuildDemoList()
{
	...
	{
		ReplayStreamer->EnumerateStreams(EnumerateStreamsVersion, FString(), FString(), FOnEnumerateStreamsComplete::CreateSP(this, &SAmethystDemoList::OnEnumerateStreamsComplete));
	}
}

/** Called when demo list building is finished */
void SAmethystDemoList::OnBuildDemoListFinished()
{
	bUpdatingDemoList = false;
...
void SAmethystDemoList::PlayDemo()
{
...
void SAmethystDemoList::DeleteDemo()
{
	if (bUpdatingDemoList)
	{
		// We're still building the list
	...
	if (SelectedItem.IsValid())
	{
		...
		if ( GI != NULL )
		{
			UAmethystForestViewportClient* AmethystViewport = Cast<UAmethystForestViewportClient>( GI->GetGameViewportClient() );
			...
			if ( AmethystViewport )
			{
				AmethystViewport->ShowDialog(
					...
					FOnClicked::CreateRaw(this, &SAmethystDemoList::OnDemoDeleteConfirm),
					FOnClicked::CreateRaw(this, &SAmethystDemoList::OnDemoDeleteCancel)
				);
			}
		}
	}
...
FReply SAmethystDemoList::OnDemoDeleteConfirm()
{
	if (SelectedItem.IsValid() && ReplayStreamer.IsValid())
	{
		...
		ReplayStreamer->DeleteFinishedStream(SelectedItem->StreamInfo.Name, FOnDeleteFinishedStreamComplete::CreateSP(this, &SAmethystDemoList::OnDeleteFinishedStreamComplete));
	}
...
FReply SAmethystDemoList::OnDemoDeleteCancel()
{
	UAmethystForestInstance* const GI = Cast<UAmethystForestInstance>(PlayerOwner->GetGameInstance());
...
void SAmethystDemoList::OnDeleteFinishedStreamComplete(bool bWasSuccessful)
{
	BuildDemoList();
}

void SAmethystDemoList::OnFocusLost(const FFocusEvent& InFocusEvent)
{
...
FReply SAmethystDemoList::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
	return FReply::Handled().SetUserFocus(SharedThis(this), EFocusCause::SetDirectly, true);
}

void SAmethystDemoList::EntrySelectionChanged(TSharedPtr<FDemoEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void SAmethystDemoList::OnListItemDoubleClicked(TSharedPtr<FDemoEntry> InItem)
{
	SelectedItem = InItem;
	PlayDemo();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void SAmethystDemoList::MoveSelection(int32 MoveBy)
{
...
FReply SAmethystDemoList::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyboardEvent)
{
...
TSharedRef<ITableRow> SAmethystDemoList::MakeListViewWidget(TSharedPtr<FDemoEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
