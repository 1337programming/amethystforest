...
void FAmethystOptions::Construct(ULocalPlayer* InPlayerOwner)
{
...
void FAmethystOptions::InfiniteAmmoOptionChanged(TSharedPtr<FAmethystMenuItem> MenuItem, int32 MultiOptionIndex)
{
	...
	if (World)
	{
		...
		{
			AAmethystPlayerController* AmethystPC = Cast<AAmethystPlayerController>(*It);
			if (AmethystPC)
			{
...
void FAmethystOptions::InfiniteClipOptionChanged(TSharedPtr<FAmethystMenuItem> MenuItem, int32 MultiOptionIndex)
{
	...
	if (World)
	{
		for (FConstPlayerControllerIterator It = World->GetPlayerControllerIterator(); It; ++It)
		{
			AAmethystPlayerController* const AmethystPC = Cast<AAmethystPlayerController>(*It);
			if (AmethystPC)
			{
...
{
	UWorld* const World = PlayerOwner->GetWorld();
	AAmethystForestState* const GameState = World ? Cast<AAmethystForestState>(World->GameState) : nullptr;
	if (GameState)
	{
