	...
	if (URL == TEXT("/index.html?scoreboard"))
	{
		...
		if (GameEngine)
		{
			...
			{			
				...
				{
					// get the shoter game
					AAmethystForestState* const GameState = Cast<AAmethystForestState>(Player->PlayerController->GetWorld()->GameState);
...
void InitializeAmethystForestDelegates()
{
