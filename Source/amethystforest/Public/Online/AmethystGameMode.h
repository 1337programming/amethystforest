...
#include "AmethystForestMode.generated.h"

class AAmethystAIController;
class AAmethystPlayerState;
class AAmethystPickup;
class FUniqueNetId;

UCLASS(config=Game)
class AAmethystForestMode : public AGameMode
{
	...
	virtual bool CanDealDamage(AAmethystPlayerState* DamageInstigator, AAmethystPlayerState* DamagedPlayer) const;
	...
	/** Create a bot */
	AAmethystAIController* CreateBot(int32 BotNum);
...
protected:
	...
	virtual void InitBot(AAmethystAIController* AIC, int32 BotNum);

	/** check who won */
	virtual void DetermineMatchWinner();

	/** check if PlayerState is a winner */
	virtual bool IsWinner(AAmethystPlayerState* PlayerState) const;
...
public:	
	...
	UPROPERTY()
	TArray<AAmethystPickup*> LevelPickups;
