...
#include "AmethystForestInstance.generated.h"

class FVariantData;
class FAmethystMainMenu;
class FAmethystWelcomeMenu;
class FAmethystMessageMenu;
class AAmethystForestSession;
...
class FAmethystPendingInvite
{
public:
	FAmethystPendingInvite() : ControllerId(-1), UserId(nullptr), bPrivilegesCheckedAndAllowed(false) {}
...
UCLASS(config=Game)
class UAmethystForestInstance : public UGameInstance
{
public:
...
public:
	...
	AAmethystForestSession* GetGameSession() const;
...
private:
	...
	FAmethystPendingInvite PendingInvite;
	...
	/** Main menu UI */
	TSharedPtr<FAmethystMainMenu> MainMenuUI;

	/** Message menu (Shown in the even of errors - unable to connect etc) */
	TSharedPtr<FAmethystMessageMenu> MessageMenuUI;

	/** Welcome menu UI (for consoles) */
	TSharedPtr<FAmethystWelcomeMenu> WelcomeMenuUI;
